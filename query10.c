...
int Filter(Student s){
    for(int i=0;i<90;i++){
        if(s.languages[i]=='L'&&s.languages[i+1]=='i'&&s.languages[i+2]=='t'&&s.languages[i+3]=='h'&&s.languages[i+4]=='u'&&s.languages[i+5]=='a'&&s.languages[i+6]=='n'&&s.languages[i+7]=='i'&&s.languages[i+8]=='a'&&s.languages[i+9]=='n') return 1;
    }
    return 0;
}
