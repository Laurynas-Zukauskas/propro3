...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        int size = 0;
        float maks=0;                  // how many students in database
        ...
        for (int i = 0; i < size; i++){
            ...
            if(students[i].average>maks) maks=students[i].average;
        }
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            if(s.average>=maks-1){ // *** first filter, conditions on the student
